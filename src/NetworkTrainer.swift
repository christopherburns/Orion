...
public struct NetworkTrainer {
   ...
   struct TrainReport: Encodable {
      ...
      struct Parameters: Encodable {
         // Encoding era of the binary (and therefore of the trained model).
   ...
   public static func trainModel (
      ...
      // Training data files have no architecture version — only a raw state
      // dimension in their header. Data from a different encoding era (e.g. a
      // pre-v9 file lacking the gem-headroom feature) has the wrong per-example
      // length and would otherwise crash deep inside an MLX reshape with no
      // useful message. Fail clearly here instead, before any batching.
      if let firstStateDim = allExamples.first?.state.count, firstStateDim != PolicyValueNetwork.INPUT_DIMENSIONS {
         throw NSError(domain: "NetworkTrainer", code: 1, userInfo: [NSLocalizedDescriptionKey:
            "Training data state dimension (\(firstStateDim)) does not match this binary's " +
            "expected input dimension (\(PolicyValueNetwork.INPUT_DIMENSIONS)). This data was " +
            "generated by a different encoding era (architecture version) — regenerate it with " +
            "the current `orion generate`, or train against data from a matching build."])
      }
